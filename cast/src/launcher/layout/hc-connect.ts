...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    : err === ERR_HASS_HOST_REQUIRED
      ? "Please enter a MSH URL."
      : err === ERR_INVALID_HTTPS_TO_HTTP
        ? html`
            Cannot connect to MSH instances over "http://". ${seeFAQ("https")}
          `
        : `Unknown error (${err}).`;

const INTRO = html`
  <p>
    MSH Cast allows you to cast your MSH installation to Chromecast video
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your MSH URL and click authorize. If you
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a MSH URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
